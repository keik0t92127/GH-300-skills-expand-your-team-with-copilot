...
def get_activities(
    ...
    - difficulty: Filter activities by difficulty level ('Beginner', 'Intermediate', 'Advanced', or 'none' for activities with no difficulty set)
    """
    ...
    if difficulty == "none":
        ...
        if difficulty not in allowed_difficulties:
            raise HTTPException(status_code=400, detail=f"Invalid difficulty. Must be one of: {', '.join(sorted(allowed_difficulties))}")
